...
# Common paths to help locate CSVs when run from different working dirs
SCRIPT_DIR = os.path.abspath(os.path.dirname(__file__))
REPO_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, "..", ".."))
DATASET_DIRS = [
    os.path.join(REPO_ROOT, "Dataset-Umut"),
    os.path.join(REPO_ROOT, "datasets"),
    SCRIPT_DIR,  # Labs-Umut/lab3
    ".",         # current working dir
]
...
def safe_read_csv(path: str) -> pd.DataFrame:
    ...
    if not os.path.isabs(path):
        ...
        candidates.extend([os.path.join(d, os.path.basename(path)) for d in DATASET_DIRS])
    ...
    for p in candidates:
        if os.path.exists(p):
            try:
                ...
                df = pd.read_csv(p, parse_dates=True, infer_datetime_format=True) #file oku ve datetime kolonlarını otomatik olarak tahmin et
                break
            except Exception as e:
                last_err = e
                print(f"[ERROR] Could not read {p}: {e}")
    else:
        print(f"[WARN] File not found in candidates: {candidates}")
        return df

    if df.empty and last_err:
        print(f"[WARN] CSV read yielded empty dataframe. Last error: {last_err}")
...
def compute_wind_speed(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["wind_speed_10m"] = np.sqrt(df[u_col] ** 2 + df[v_col] ** 2)  # u10m ve v10m'nin karelerinin toplamının karekökü (hız büyüklüğü)
...
def main(args: Optional[list[str]] = None) -> int: #string formatlı liste girecek 
    ...
    parser.add_argument("--savefigs", action="store_true", default=True, help="Save figures to PNG files (default: on)") #savefigs: grafikleri PNG dosyasına kaydet.
    ...
    # --- Plots
    # Figürleri repo içindeki Labs-Umut/lab3 klasöründe "figures" altına at
    ...
    if parsed.savefigs:
        ...
        print(f"[INFO] Figures will be saved to: {outdir}")
